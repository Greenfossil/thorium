...
class TokenUtilSuite extends FunSuite {
  ...
  test("roundtrip token - happy path") {
    ...
    val res = for {
      token <- TokenUtil.generateToken(value, duration, notBefore, passwordKey, AESUtil.AES_GCM_NOPADDING)
  ...
  test("malformed token fails gracefully") {
    ...
    val res = TokenUtil.verifyToken(bad, passwordKey, AESUtil.AES_GCM_NOPADDING)
    assert(res.isFailure)
  }
  ...
  test("mismatched algorithm fails verification") {
    ...
    val res = for {
      token <- TokenUtil.generateToken(value, duration, notBefore, passwordKey, AESUtil.AES_GCM_NOPADDING)
      verified <- TokenUtil.verifyToken(token, passwordKey, AESUtil.AES_CTR_NOPADDING)
    } yield verified
