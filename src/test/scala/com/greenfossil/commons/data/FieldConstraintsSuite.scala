...
class FieldConstraintsSuite extends munit.FunSuite {
  ...
  test("number"){
    ...
    val errorField = f.bind(Map("f" -> "20"))
    assertEquals(errorField.errors.size, 1)
    errorField.errors.foreach(e => println(e.message))

    val validField = f.bind(Map("f" -> "5"))
  ...
  test("text with trim option"){
    ...
    val validField = f.bind(Map("f" -> "hello"))
    assert(validField.errors.isEmpty)

    val errorField = f.bind(Map("f" -> "hello world"))
    assertEquals(errorField.errors.size, 1)
    errorField.errors.foreach(e => println(e.message))

    val errorField2 = f.bind(Map("f" -> "    "))
    ...
    val validField2 = f.bind(Map("f" -> "hello "))
  ...
  test("text without trim option"){
    ...
    val errorField2 = f.bind(Map("f" -> "hello "))
    assertEquals(errorField2.errors.size, 1)
    errorField.errors.foreach(e => println(e.message))
  ...
  test("byte number"){
    ...
    val errorField = f.bind(Map("f" -> 10.toByte.toString))
    assertEquals(errorField.errors.size, 1)
    errorField.errors.foreach(e => println(e.message))

    val validField = f.bind(Map("f" -> "8"))
    assert(validField.errors.isEmpty)
  }
  ...
  test("short number"){
    ...
    val nonStrictValidField = nonStrictField.bind(Map("f" -> "1"))
    assertEquals(nonStrictValidField.errors.size, 0)

    val nonStrictErrorField = nonStrictField.bind(Map("f" -> "0"))
    ...
    val strictValidField = strictField.bind(Map("f" -> "2"))
    assertEquals(strictValidField.errors.size, 0)

    val strictErrorField = strictField.bind(Map("f" -> "0"))
  ...
  test("big decimal"){
    ...
    val errorField = field.bind(Map("f" -> "1")) // scale is 0, precision is 1
    assertEquals(errorField.errors.size, 1)
    assertEquals(errorField.errors.head.message, "error.real.precision")

    val validField = field.bind(Map("f" -> "0.1")) // scale is 1, precision is 1
